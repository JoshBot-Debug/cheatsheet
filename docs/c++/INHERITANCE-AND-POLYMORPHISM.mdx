...
::::note
This is apparently debatable. Most people think of **composition** as a *Has-a* relationship. If someone asks you what is a *Has-a* relationship, say **composition** before private inheritance.
::::
...
public:
    int derived_var = 2;
    void print() const override { std::cout << "I am Derived\n"; }
};
...
int main() {
    ...
    // Object slicing
    Base b = d; 
    b.print(); // "I am Base" (Derived behavior is lost)
    // std::cout << b.derived_var; // Compiler Error! Member does not exist in 'b'

    callByValue(d); // Outputs: "I am Base"
}
```

When `Base b = d` `Base` a new stack member is created and enough memory is allocated for `Base`. `Base` copy constructor is called. `Derived d` is left untouched it's memory is intact. 
When object slicing occurs, the virtual pointer (vptr) is updated by the compiler to point to the `Base` class's virtual table (vtable). It does not keep pointing to the `Derived` table. 
...
graph TD;
    ...
    class S1,S2 speedNode;
    class BE,E,F birdNode;
```
